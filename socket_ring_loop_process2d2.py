...
#      f=open('file.bin','rb')
#      dd=f.read()
#      nblock=len(dd)/1024/16/2
#      data = np.frombuffer(dd, dtype=np.float16).reshape((nblock,1024,16))
#      XXX data=np.ones((len(dd)/1024/16/2,1024,16),dtype='float32')
#      XXX data[:,:,:]=np.frombuffer(dd,dtype='<i2').reshape(len(dd)/1024/16/2,1024,16)
#      
...
itime = 0
dtime = 100    # save ringbuffer for every dtime sec.
sn=600.         # triggering SN

BLK_SIZE = Nx*Ny*Nz   #1000*1024*16
NBLK = 20           # NBLK*NX is power of 16 for bonsai only
njump = 80
HDR_SIZE = 64
nchunk=Nx*NBLK
...
#   print(data[0,id])
    ...
    # run it as a simple visual sanity check.
        plot_writer = bonsai.trigger_plot_writer('test_trigger_plot', nchunk) # length of x, 1024
...
#       exit(1)
    ...
    if global_max_trigger > sn:
        ...
        #print('np.save takes %f sec') %(time.time()-ts)
    else:
...
#   time.sleep(3)
    ...
    if id == 2 and (itime % dtime) == 0 :
        ...
        #for it in range(int(t_total)):
           #print('trigger sn=',global_max_trigger)
            #sav_intensity[:,:,it]=ring[it*nbeam+id-1]
        #print('t_total=%f sec, copy from ringbuffer takes %f sec') %(t_total,time.time()-ts)    
        ts=time.time()
        path='/mnt/ramdisk/'
        #np.save(path+timestr+'_beam'+str(id)+'.npy',sav_intensity)
        #print('np.save takes %f sec') %(time.time()-ts)
...
# check write buffer
    ...
    data1[nb*Nx:(nb+1)*Nx,:,:] = np.frombuffer(buffer, dtype=np.float16).reshape((Nx, Ny, Nz))  
